...
def getFirstAvailableDevice():  # used to return the first available device running spotify (more commonly browser,desktop or mobile)
    from SMITY.urlHandling.openUrls import \
        openUrl  # used to open spotify web page in case it or the spotify desktop app is not already open
...
def searchForId(q,
                idtype):  # used to search for an id in the spotify database given a query e.g song name and a type e.g track
    try:
        sp = spotifyAuth()
