...
city = 'johannesburg'                         # variable for the city that I want to choose

API_key = ""               #DISCLAIMER I did not include my API_key , you can get yours at openweather.org
...
#similar error would be 429       because of:      More that 60 API calls per minute
  ...
  temp_min =  ((API_data['main']['temp_min'])- 273.15)      #same routine for the minimum temprature as with temp_city


  temp_max = ((API_data['main']['temp_max'])- 273.15)       #same routine for the maximum temprature as with temp_city


  weather_desc = ((API_data['weather'][0]['description']))  #a dictionary which has only a single array element 0 and inside of it nests another dictionary


  humidity = ((API_data['main']['humidity']))     #same procedure as the temp_city


  date_time = datetime.now().strftime("%d %b %Y | %I:%M:%S %p")  #datetime.now gives us the time and strftime helps convert the date and time objects to string
  ...
  print("Today's forecast in {} {}".format(city,date_time)) #format puts the variables into the "{}" from left to right

  print("--------------------------------------------------------------")

  print("The temprature is: {:.2f} °C".format(temp_city))      #.2f is the amount of the demicals it is allowed to print
